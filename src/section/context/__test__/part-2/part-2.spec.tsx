...
import ContextPart2 from '../../part-2';
...
describe(`Testing Part 2 Context Section`, () => {
  ...
  it(`Snapshot Testing`, async () => {
    const { container } = render(<ContextPart2 />);

    expect(container).toMatchSnapshot();
  });

  it(`Cell Item Should Be Updated`, async () => {
    const { getAllByTestId } = render(<ContextPart2 />);
  ...
  it(`All Cell Item Should Be Updated`, async () => {
    const { findByText, getAllByTestId } = render(<ContextPart2 />);
