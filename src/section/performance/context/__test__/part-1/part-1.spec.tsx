...
import ContextPart1 from '../../part-1';
...
describe(`Testing Part 1 Context Section`, () => {
  ...
  it(`Snapshot Testing`, async () => {
    const { container } = render(<ContextPart1 />);

    expect(container).toMatchSnapshot();
  });

  it(`Cell Item Should Be Updated`, async () => {
    const { getAllByTestId } = render(<ContextPart1 />);
  ...
  it(`All Cell Item Should Be Updated`, async () => {
    const { findByText, getAllByTestId } = render(<ContextPart1 />);
