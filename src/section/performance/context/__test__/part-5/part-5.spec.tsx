...
import ContextPart5 from '../../part-5';
...
describe(`Testing Part 4 Context Section`, () => {
  ...
  it(`Snapshot Testing`, async () => {
    const { container } = render(<ContextPart5 />);

    expect(container).toMatchSnapshot();
  });

  it(`Cell Item Should Be Updated`, async () => {
    const { getAllByTestId } = render(<ContextPart5 />);
  ...
  it(`All Cell Item Should Be Updated`, async () => {
    const { findByText, getAllByTestId } = render(<ContextPart5 />);
